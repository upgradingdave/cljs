...
(defn cell 
  [!state path-to-cell
   & [{:keys [cell-width cell-height font-size gutter-size click-fn] 
       :as opts
  ...
  [!state bingo-path
   & [{:keys [cell-width cell-height gutter-size font-size click-fn]
       ...
       :or {cell-width  b/default-cell-width
            ...
            gutter-size b/default-gutter-size
            font-size   b/default-font-size}}]]
  (let [boards-path (conj bingo-path :players :boards)
        boards      (get-in @!state boards-path)
